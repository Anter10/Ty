window.__require = function e(t, n, r) {
  ...
  Constants: [ function(require, module, exports) {
    ...
    var ShareConfig = {
      ...
      RECOVER_GAME_SHARE: [ "recovergameshare", true ],
    ...
    cc.Class({
      ...
      hideOtherSelected: function hideOtherSelected(selectItem) {
        for (var itemindex = 0; itemindex < this.luckitems.length; itemindex++) {
          ...
          "undefined" != tdata && tdata.id != selectItem.getData().id && itemsceipt.unselected();
        }
        ...
        canvas.toTempFilePath({
          ...
          success: function success(res) {
            tywx.ado.Utils.saveImage2PhoneByUrl(res.tempFilePath, function() {
              ...
              tywx.ado.Utils.showWXToast("\u9886\u53d6\u6210\u529f", 1e3);
            }, function() {
  ...
  celltile: [ function(require, module, exports) {
    ...
    cc.Class({
      ...
      showHG: function showHG() {
        var renum = this.scn;
        if (renum && 0 != renum) if (renum < tywx.ado.Constants.GameCenterConfig.moreThanTenNumber || renum % 2 != 0) {
          this.hideThanTenIcon();
          if (0 != this.prenum) {
            console.log("\u524d\u4e00\u4e2a\u503c = " + this.prenum);
            this.prenum >= tywx.ado.Constants.GameCenterConfig.moreThanTenNumber && this.prenum % 2 == 0 && tywx.gamecenter.removexhgNumber(this.prenum);
            this.prenum = 0;
          }
        } else if (false == tywx.gamecenter.hasShowxhgNum(renum, this.id) && false == tywx.gamecenter.hadXHGId(renum)) {
          this.showThanTenIcon();
          this.prenum = renum;
          tywx.gamecenter.addxhgNumber(renum, this.id);
          tywx.gamecenter.showmsm1();
        } else true == tywx.gamecenter.hadXHGId(renum) && this.hideThanTenIcon();
      },
  ...
  gamemain: [ function(require, module, exports) {
    ...
    var gamemain = cc.Class({
      ...
      removexhgNumber: function removexhgNumber(num) {
        for (var thn = 0; thn < this.curshowxhgs.length; thn++) if (this.curshowxhgs[thn].num == num) {
      ...
      showRYBoxButton: function showRYBoxButton() {
        if (this.score >= tywx.config.ruyiScore && false == this.ryBoxBtn.active) if (true == tywx.config.auditing) this.ryBoxBtn.active = false; else {
          this.ryBoxBtn.active = true;
          var animation = this.ryBoxBtn.getComponent(cc.Animation);
          animation.play("daiji");
        } else this.score < tywx.config.ruyiScore && (this.ryBoxBtn.active = false);
      },
      ...
      showItem: function showItem() {
        ...
        var is_ipx = ch / cw >= 1.9;
        is_ipx && (this.itemview.y = this.scoreLabel.node.y - 30);
        for (var itemIndex = 0; itemIndex < tywx.ado.Constants.GameCenterConfig.allitem.length; itemIndex++) {
          ...
          itemsceipt.setClickCall(function(data, item) {
            if (self.gamestate == tywx.ado.Constants.GameCenterConfig.gameState.waitclick) if (data.num > 0) {
              var canupdate = true;
              if (4 == data.id) if (self.point == tywx.ado.Constants.GameCenterConfig.maxphy_value) {
      ...
      showOneHpEf: function showOneHpEf(item) {
        if ((5 != this.point || 1 != this.point) && 0 == item.xx1Icon.node.getNumberOfRunningActions()) {
      ...
      drawPhyPoint: function drawPhyPoint() {
        ...
        for (var starIndex = 0; starIndex < 5; starIndex++) this.point >= starIndex + 1 ? this.allstars[starIndex].getComponent("LifeStar").show() : this.allstars[starIndex].getComponent("LifeStar").hide();
      },
      ...
      initgame: function initgame(restart) {
        ...
        this.scoreLabel.string = this.score;
        this.point = -1 === tmp_progress ? tywx.ado.Constants.GameCenterConfig.maxphy_value : tmp_progress.life;
        this.drawPhyPoint();
      ...
      lingQuItem: function lingQuItem(dontsetnull, giveitem) {
        ...
        if (null != this.produceItem) {
          ...
          null != item && this.showAlertMSG("\u9886\u53d6\u6210\u529f " + item.name + " \u9053\u5177");
      ...
      showBox: function showBox(maxnum) {
        ...
        if (is_show) {
          ...
          var frameindex = 0;
          2 == this.produceItem.id ? frameindex = 1 : 5 == this.produceItem.id ? frameindex = 2 : 6 == this.produceItem.id && (frameindex = 3);
      ...
      showUseHpHelp: function showUseHpHelp(needaddstore) {
        if (parseInt(tywx.Util.getItemFromLocalStorage("hadshowhpnum", 0)) < 2) {
          needaddstore && tywx.Util.setItemToLocalStorage("hadshowhpnum", parseInt(tywx.Util.getItemFromLocalStorage("hadshowhpnum", 0)) + 1);
        ...
        } else this.hpHelpTips.active = false;
      },
      ...
      btnRefreshGameOverPhotoCallback: function btnRefreshGameOverPhotoCallback() {
        var photo_urls = tywx.config.CDNImages.GameOverPhotos;
        (!this.showPicIndex || this.showPicIndex > photo_urls.length - 1) && (this.showPicIndex = 0);
        var pngurl = tywx.SystemInfo.cdnPath + "share_pyq/addone/" + photo_urls[this.showPicIndex];
        console.log("cdnpngurl = " + pngurl);
    ...
    var gamestart = cc.Class({
      ...
      refreshAfterGetConfig: function refreshAfterGetConfig() {
        if (!tywx.config.auditing) {
          ...
          adNode ? adNode.showAdNode() : tywx.AdManager.showAd(cc.v2(100, 100), "GAME_START");
        }
      },
      ...
      startGame: function startGame() {
        ...
        -1 != this.haveStoreGameData ? this.haveStoreGameData.score >= tywx.ado.Constants.GameCenterConfig.prePlayGetScore ? this.showGoonProgress() : this.goonProgressGame(true) : cc.director.loadScene("gamemain");
      },
...
}, {}, [ "AdManager", "BiStatLog", "Bilog", "EncodeDecode", "EventType", "HttpUtil", "NotificationCenter", "PropagateInterface", "ShareInterface", "TCPClient", "Timer", "TuyooSDK", "Util", "boot", "gamemain", "gamestart", "AddOneConfig", "AddScoreLabel", "Audio", "AudioManager", "Constants", "DjItem", "Effect", "GeZi", "GeZiData", "GeZiMask", "ItemHelp", "LifeStar", "LingQuPageView", "LuckBox", "MoreTanNumber", "ShareButton", "UserItem", "Utils", "celltile", "helpview", "luckitem", "phbview" ]);
