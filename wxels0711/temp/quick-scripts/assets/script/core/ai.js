(function() {"use strict";var __module = CC_EDITOR ? module : {exports:{}};var __filename = 'preview-scripts/assets/script/core/ai.js';var __require = CC_EDITOR ? function (request) {return cc.require(request, require);} : function (request) {return cc.require(request, __filename);};function __define (exports, require, module) {"use strict";
cc._RF.push(module, '4c6b3m3u6JNCoOx4Mg2Df/Y', 'ai', __filename);
// script/core/ai.js
...
var ElsAi = Class.extend({
    ...
    //布局评分,用于AI计算最合理的摆法...
    getGridScore: function getGridScore(tg, cx, cy, cf, nf, ccombo) {
        var core = tg.mcore;
        ...
        for (i = 0; i < els.HENG; i++) {
            ...
            xiagu[i] = 0;
            if (i == 0) {
                if (core.col_top[1] > core.col_top[0]) xiagu[i] = core.col_top[1] - core.col_top[0];
            } else if (i == els.HENG - 1) {
                if (core.col_top[i - 1] > core.col_top[i]) xiagu[i] = core.col_top[i - 1] - core.col_top[i];
            } else {
                if (core.col_top[i + 1] > core.col_top[i] && core.col_top[i - 1] > core.col_top[i]) xiagu[i] = Math.min(core.col_top[i - 1], core.col_top[i + 1]) - core.col_top[i];
            }
            if (xiagu[i] > 2) xiagu_count++;
            xiagu_total += xiagu[i];
        }
        ...
        //连击加分
        if (ccombo > 2) score += ccombo * aip["combo"][0];else score += ccombo * aip["combo"][1];

        //连击加分
        if (core.combo > 2) score += core.combo * aip["combo"][0];else score += core.combo * aip["combo"][1];

        //峡谷越少越好，有一个大峡谷不怕，怕出现两个峡谷
        if (xiagu_count >= aip["xiagu.max"]) score += aip["xiagu"] * xiagu_total;

        return score;
    },

    AI_F1: function AI_F1(tg, cx, cy, cf, combo, fs, save, scan) {
        ...
        bq = this.tActQueue;
        var tmpz;
        if (els.ELS_CLASSIC) tmpz = els.ZCOUNT_C[tg.mcore.cur_block];else tmpz = els.ZCOUNT_NC[tg.mcore.cur_block];
        ...
        for (var s2 = 0; s2 <= tmpsave; s2++) {
            ...
            for (var nz = 0; nz < tmpz; nz++) {
                ...
                for (var x2 = 0; x2 < 3; x2++) {
                    ...
                    this.tActQueue = bq2;
                    //左移
                    if (x2 == 1) while (tg.moveBlk(els.LEFT, true) != els.REACH_BORDER) {
                        this.tActQueue += 'L';
                        fs(this, tg, cx, cy, cf, combo, scan);
                    }
                    //右移
                    if (x2 == 2) while (tg.moveBlk(els.RIGHT, true) != els.REACH_BORDER) {
                        this.tActQueue += 'R';
                        fs(this, tg, cx, cy, cf, combo, scan);
                    }
                    if (x2 == 0) fs(this, tg, cx, cy, cf, combo, scan);
                }
                b3.recycle();
            }
            b2.recycle();
    ...
    AI_F2: function AI_F2(tai, tg, cxs, cys, cfs, combo, scan) {
        var cx, cy, cf, ccombo;
        var bq0 = '';
        ...
        if (scan) {
            tai.ms_scan[tai.ms_scan.length] = [s, tai.tActQueue, tg.mcore, cx, cy, cf, ccombo];
        } else {
            if (s > tai.maxscore_scan) tai.AI_F1(tg, cx, cy, cf, ccombo, tai.AI_F3);
        }
    ...
    AI_F3: function AI_F3(tai, tg, cx, cy, cf, combo, scan) {
        var s, nf;
        var b;
        var bq = '';
        b = tg.mcore.clone();
        ...
        for (var i = 0; i < 4; i++) {
            ...
            if (xx >= els.HENG) break;
            if (tg.mcore.col_top[xx] > mx) mx = tg.mcore.col_top[xx];
        }
    ...
    getAIAct: function getAIAct(tg) {
        ...
        if (len == 0 && tai.work2idx < 0) {
            ...
            //高分在前面
            tai.ms_scan.sort(function _(a, b) {
                return b[0] - a[0];
            });
