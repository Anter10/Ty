...
// //应用系统信息
// tywx.SystemInfo = {
//     clientId: 'H5_5.1_weixin.weixin.0-hall20016.weixin.elsfk',
//     intClientId: 24099,
//     cloudId:8,
//     version:0.01,
//     webSocketUrl: 'wss://waxiaofzwss.nalrer.cn',
//     loginUrl : 'https://openwaxiaofz.nalrer.cn/', // 仿真
//     shareManagerUrl : 'https://market.touch4.me/',
//     deviceId: 'wechatGame',
//     wxAppId: 'wx7e3b12072efbfc27',
//     appId: 9999,
//     gameId: 20016,
...
//应用系统信息
tywx.SystemInfo = {
    clientId: 'H5_5.1_weixin.weixin.0-hall20082.weixin.elsfkwlb',
    intClientId: 24132,
    cloudId:36,
    version:3.52,
    webSocketUrl: 'wss://elsfkwss.nalrer.cn',
...
tywx.UserInfo = {
    ...
    setItemCount: function (server_key, count) {
        var ret = false;
        if (count === 1 && tywx.UserInfo.getPropCountByKey(tywx.UserInfo.server2local(server_key)) === 0) {
            tywx.UserInfo.bag.push({kindID:server_key, itemId:server_key, count: 1});
            return true;
        }
        
        var s_k = server_key;
        for(var i = 0; i < tywx.UserInfo.bag.length; i++){
            var item = tywx.UserInfo.bag[i];
            if(item.kindID === s_k) {
                if (item.count + count < 0) {
                    ret = false;
                } else {
                    item.count = item.count + count;
                    tywx.UserInfo.bag[i] = item;
                    ret = true;
                }
                break;
            }
    ...
    getPropCountByKey:function(local_key){
        var s_k = tywx.UserInfo.local2Server(local_key);
        var ret = 0;
        for(var i = 0; i < tywx.UserInfo.bag.length; i++){
            var item = tywx.UserInfo.bag[i];
            if(item.kindID === s_k) {
        ...
        /// 没有取到
        if(ret === 0) console.log('GlobalInit/getPropCountByKey','get prop count by key error', local_key);
        return ret;
    },
};
