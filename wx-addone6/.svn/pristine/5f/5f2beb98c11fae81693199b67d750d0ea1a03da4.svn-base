 cc.Class({
     ...
     storePhoto: function () {
         if (this.hadstorephotonumber) {
             ty.ado.Utils.showWXToast("已经保存成功", 500);
             return;
         }
         ...
         canvas.toTempFilePath({
             ...
             success: function success(res) {
                 //console.log(res);
                 ty.ado.Utils.saveImage2PhoneByUrl(res.tempFilePath, function () {
                     //  self.hadstorephotonumber = true;
                     // 注释保存图片宋道具
                     // self.storesuccall && self.storesuccall();
                     //  ty.ado.Utils.showWXToast("领取成功", 1000);
                     ty.ado.Utils.showWXToast("保存图片成功", 1000);
                 }, function () {
                     self.hadstorephotonumber = false;
                     ty.ado.Utils.showWXToast("保存图片失败");
                 });
             },
     ...
     closeCall: function () {
         this.close();
         if (ty.gamecenter && ty.gamecenter.shareDoubleScore) {
             ty.gamecenter.shareDoubleScore(this.addscore);
         }
     },
     ...
     doubleScore: function () {
         if (ty.gamecenter && ty.gamecenter.shareDoubleScore) {
             ty.gamecenter.shareDoubleScore(this.addscore * 2);
         }
     },
     ...
     uiAni: function () {
         this.doubleAddInit();
         ty.ado.Utils.commonScaleIn(this.rootNode);
     },
     ...
     doubleAddInit: function () {
         const share_control = ty.config.share_control_2.bigdouble;
         var random = parseInt(Math.random() * 100);
         if ((share_control[0] == "video" || ty.ado.isMinGanIP) && ty.ado.isCanWatchVideo) {
             this.shareButton.getComponent("ShareButton").setShareConfig(ty.ado.Constants.ShareConfig.FREE_DOUBLE_SCORE_SHARE_VIDEO);
             if (random <= share_control[1] || ty.ado.isMinGanIP) {
                 this.shareButton.getComponent("ShareButton").setButtonCallType(2);
             } else {
                 this.shareButton.getComponent("ShareButton").setShareConfig(ty.ado.Constants.ShareConfig.FREE_DOUBLE_SCORE_SHARE);
     ...
     btnRefreshBgPhotoCallback() {
         let photo_urls = ty.config.PYQImg.MergeBigNumPhotos;
         if (!this.showPicIndex || this.showPicIndex > (photo_urls.length - 1)) {
             this.showPicIndex = 0;
         }
         let pngurl = ty.SystemInfo.cdnPath + 'share_pyq/addone/' +
         ...
         ty.ado.Utils.refreshSpriteByUrl(sprite, pngurl);
