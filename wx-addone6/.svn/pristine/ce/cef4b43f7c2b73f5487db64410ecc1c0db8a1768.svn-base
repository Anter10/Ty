...
ty.Util = {
    ...
    checkLocalUUID: function() {
        var local_uuid = ty.Util.getItemFromLocalStorage("LOCAL_UUID_KEY", "");
        return local_uuid != "";
    },
    ...
    getLocalUUID: function() {
        var local_uuid = ty.Util.getItemFromLocalStorage("LOCAL_UUID_KEY", "");
        if (!local_uuid){
            local_uuid = ty.Util.createUUID();
            ty.Util.setItemToLocalStorage("LOCAL_UUID_KEY", local_uuid);
        }
        return local_uuid;
    },
    ...
    getItemFromLocalStorage: function (keyStr, defaultValue) {
        ...
        var v = cc.sys.localStorage.getItem(keyStr);
        if (!v) {
            v = defaultValue;
            for (let i = 0; i < 3; i++) {
                let tmp_v = cc.sys.localStorage.getItem(keyStr);
                if (!tmp_v) continue;
                v = tmp_v;
                break;
            }
    ...
    setItemToLocalStorage: function (keyStr, ValueStr) {
        ...
        try {
            ...
            ty.LOGE("ty.Util", "setItemToLocalStorage fail");
        }
    },
};
