...
ty.clickStatEventType = {
    /**
     * ty.SystemInfo.errorLogServer = "https://clienterr.touch4.me/api/bilog5/clientlog"
     * ty.SystemInfo.biLogServer = "https://cbi.touch4.me/api/bilog5/text"
     * 使用方法:
     * 将bilog.js加入工程后,在需要打点的地方调用ty.BiLog.clickStat(事件id,[参数列表]);
...
ty.BiLog = {
    ...
    uploadLogTimely:function (logtxt) {
        if(!ty.StateInfo.networkConnected) {
            ty.LOGD('ty.BiLog', 'net error!');
            return;
        }
        if(logtxt) {
            var header = ['Content-Type:text/plain'];
            var configObj = {
                'url': ty.SystemInfo.errorLogServer + '?cloudname=' + ty.SystemInfo.cloudId + '.' + ty.SystemInfo.intClientId,
                'header': header,
                'postData': logtxt,
                'callback': null
            };
    ...
    uploadClickStatLogTimely:function (logtxt) {
        ...
        ty.ado.logWithColor('BILOG===>'+logtxt);
        ty.HttpUtil.httpPost(configObj,'POST');
    },
    ...
    clickStat: function (eventId, paramsList) {
        ...
        ty.LOGD('BI打点', "eventid= " + eventId + " 描述 = " + JSON.stringify(dyeparams));
