...
ty.AdManager = {
    ...
    showAd: function(position, tag) {
        ...
        var _adnode = new ty.AdManager.adNodeClass();
        _adnode.adInfoList = JSON.parse(JSON.stringify(ty.AdManager.allAdInfoList));
        _adnode.adType = 1;
        _adnode.adTag = tag;

        _adnode.createAdNode(position);

        ty.AdManager.adNodeList.push(_adnode);

    },
    showAd2Parent: function(parent, tag){
        var _adnode = new ty.AdManager.adNodeClass();
        _adnode.adInfoList = JSON.parse(JSON.stringify(ty.AdManager.allAdInfoList));
        _adnode.adType = 1;
        _adnode.adTag = tag;
    ...
    showBanner: function (position, tag) {
        ...
        var _adnode = new ty.AdManager.adNodeClass();
        _adnode.adInfoList = JSON.parse(JSON.stringify(ty.AdManager.allBannerInfoList));
        _adnode.adType = 2;
        _adnode.adTag = tag;

        _adnode.createAdNode(position);

        ty.AdManager.bannerNodeList.push(_adnode);
    ...
    adNodeObj  : {
        ...
        createAdNode2Parent: function(parent){
            this.genRandomFirstAdInfo();
        ...
        adIconNode : function () {
            ...
            // ty.BiLog.clickStat(ty.clickStatEventType.clickStatEventTypeShowAdBtn, [that.currentAdInfo.icon_id,
            //     '0',
            //     '',
            ...
            switch (_animaType){
                ...
                case ty.AdManager.AnimType.STATIC:
                    ...
                    break;
                case ty.AdManager.AnimType.SHAKE:
                    ...
                    break;
                case ty.AdManager.AnimType.FRAME:
        ...
        onClickAdIconBtn: function() {
            ...
            try {
                ...
                ty.BiLog.clickStat(ty.clickStatEventType.clickStatEventTypeClickShowQRCode, bi_paramlist);
    ...
    freshAdIconByTime : function () {
        ...
        ty.AdManager.adNodeList.forEach(function (_adNode) {
    ...
    freshAdBannerByTime :function () {
        ty.AdManager.bannerNodeList.forEach(function (_adNode) {
            _adNode && _adNode.resetBtnIcon && _adNode.resetBtnIcon();
        });
    },
    ...
    startFreshAdIcon :function () {
        ...
        ty.Timer.cancelTimer(cc.director, ty.AdManager.freshAdIconByTime);
        ty.Timer.setTimer(cc.director, ty.AdManager.freshAdIconByTime,icon_interval, cc.macro.REPEAT_FOREVER,icon_interval);
        ...
        ty.Timer.cancelTimer(cc.director, ty.AdManager.freshAdBannerByTime);
        ty.Timer.setTimer(cc.director, ty.AdManager.freshAdBannerByTime,banner_interval, cc.macro.REPEAT_FOREVER,banner_interval);
    ...
    getConfigSignStr: function(reqObj) {
        ...
        var finalSign = ty.hex_md5('market.tuyoo.com-api-' + signStr + '-market.tuyoo-api') || '';
        return finalSign;
    },
    ...
    requestADInfo : function () {
        try {
            ...
            ty.LOGE("error:", "ty.AdManager.requestADInfo——" + JSON.stringify(err));
        }
    },
            ...
            success: function (res) {
                if (res.statusCode == 200) {
                    if (res.data && res.data.loc) {
                        ...
                        ty.ado.isMinGanIP = ty.ado.Utils.isMinGanIp(that.ipLocInfo);
                        ty.NotificationCenter.trigger(ty.ado.Events.ADO_EVENT_GET_IP_SUCCESS,null);
                    }
...
ty.AdManager.adNodeClass.prototype = ty.AdManager.adNodeObj;
