...
ty.PropagateInterface = {
     ...
     * http://market.touch4.me/?act=api.getShareConfig&time=1421755384&game_mark=richddz&sign=a30ab1292aa5929e7f913ceed795f78c
     ...
     ty.PropagateInterface.getShareConfigInfo(param);
     */
    getShareConfigInfo: function (reqObj) {
        ...
        var successcb = function (ret) {
            ty.LOGE("GET_SHARE_CONFIG_INFO_SUCCESS", JSON.stringify(ret));
            for (var key in ret.retmsg) {
                ty.PropagateInterface.ShareConfig[key] = ret.retmsg[key];
            }
            ty.NotificationCenter.trigger(ty.EventType.GET_SHARE_CONFIG_SUCCESS, ret);
        };

        var failcb = function (ret) {
            ty.LOGE("GET_SHARE_CONFIG_INFO_FAIL", JSON.stringify(ret));
     ...
     * http://market.touch4.me/?act=api.getShareConfig&time=1421755384&game_mark=28-20015&sign=1d356c1417a1d58fcec442eb8f655a4e
     ty.PropagateInterface.getShareConfigInfoAutoWeight();
     * 返回值例子:
      {
		    "normalshare": {
    ...
    getShareConfigInfoAutoWeight: function () {
        if (this._cachedShareConfig == undefined) {
            ...
            // 本地已有分享配置,立即返回事件
            ty.NotificationCenter.trigger(ty.EventType.GET_SHARE_CONFIG_SUCCESS, this._shuffleByWeights());
        }
    },
    ...
    _doHttpGetShareConfig: function () {
        ...
        var successcb = function (ret) {
            ...
            // ty.LOGE("分享数据= "+JSON.stringify(ret.retmsg));
            if (self._pendingFlag) {
                ty.NotificationCenter.trigger(ty.EventType.GET_SHARE_CONFIG_SUCCESS, self._shuffleByWeights());
        ...
        var failcb = function (ret) {
            ...
            if(typeof(cc) != 'undefined' && cc.director) {
                ty.Timer.setTimer(cc.director, fc, 10, 0, 0);
            } else {
                setTimeout(fc, 10000);
            }
        };
