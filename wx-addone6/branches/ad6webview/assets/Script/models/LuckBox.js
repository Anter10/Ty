...
cc.Class({
    ...
    toggleChecked: function (toggle) {
        if (!toggle.isChecked) {
            ...
            this.mflqBtn.getComponent("ShareButton").setShareConfig(ty.ado.Constants.ShareConfig.GIFT_GIFT_BOX_SHARE_VIDEO);
            this.mflqBtn.getComponent("ShareButton").setButtonCallType(2);
        } else {
            this.hideLabel.string = "免费领取";
            this.showLabel.string = "免费领取";
            this.mflqBtn.getComponent("ShareButton").setShareConfig(ty.ado.Constants.ShareConfig.GIFT_GIFT_SHARE_BOX_SHARE);
    ...
    onLoad() {
        // 显示当前的所有道具
        var allitem = ty.ado.Constants.GameCenterConfig.allitem;
        var items = [];
        ...
        mflq.setSuccessCall(function () {
            ty.gamecenter.giveItems(false, self.produceItem());
            ty.gamecenter.ryBoxBtn.active = false;
            ty.gamecenter.sharedelaytime = (new Date()).getTime();
    ...
    init: function () {
        ...
        const share_control = ty.config.share_control_2.ruyibox;
        ...
        if (ty.ado.isMinGanIP || !ty.ado.isCanWatchVideo) {
